...
const Axios = axios.create({
  baseURL: `${process.env.VUE_APP_HOST_MD055}/api/`
})

Axios.interceptors.response.use((response) => {
  if (response.data) {
    if (typeof response.data.mensagem === 'string') Vue.prototype.$notificacao(response.data.mensagem)
    else if (response.data.md055 && typeof response.data.md055.mensagem === 'string') Vue.prototype.$notificacao(response.data.md055.mensagem)
...
Vue.prototype.$axiosMD055 = Axios
